...
def mv(current_path, data):
    """
    Перемещение или переименование файла/каталога (если он уже существует)

    На вход получает текущий путь и список из файлов/каталогов для перемещения и каталог назначения
    Если указан один файл/каталог и другой файл/каталог для назначения - происходит его перемиенование
    Иначе файл/каталог удаляется в изначальном месте и копируется в место назначения
    """
    # print(f"data {data}")
